...
def get_my_request(current_user: dict = Depends(get_current_user)):
    ...
    logger.debug(
        "get_my_request: user_id=%s found=%s", current_user.get("id"), bool(req)
    )
    return req
...
def create_my_request(current_user: dict = Depends(get_current_user)):
    ...
    if pending:
        logger.debug(
            "create_my_request: user_id=%s already has pending request id=%s",
            current_user.get("id"),
            pending.get("id"),
        )
    ...
    logger.info(
        "create_my_request: created request id=%s for user_id=%s",
...
def list_requests(current_user: dict = Depends(require_admin)):
    ...
    logger.info(
        "list_requests: admin_id=%s returned %s pending requests",
        current_user.get("id"),
        len(results or []),
    )
    return results
...
def approve_request(
    request_id: int, body: ApproveBody, current_user: dict = Depends(require_admin)
):
    duration = int(body.duration_hours or 0)
    if duration < 1 or duration > 72:
        raise HTTPException(
            status_code=400, detail="duration_hours must be between 1 and 72"
        )
